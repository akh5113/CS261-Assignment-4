...
{
  	/* FIXME */
	/*get the dyn arry into a heap format*/
	_buildHeap(heap);
...
void addHeap(DynArr *heap, TYPE node)
{
	...
	/*position that is the next open spot (aka the size of array)*/
	int position = sizeDynArr(heap);
	/*puts value into the next open spot*/
	addDynArr(heap, node);

	/*percolate up*/
	while (position != 0)
	{
		/*parent will equal the halfway point*/
		parent = (position - 1) / 2;
		/*compare value of the new node(position) to the value of the parent
		if the new node is less than the value of the parent node it should percolate up*/
		if (compare(getDynArr(heap, position), getDynArr(heap, parent)) == -1)
		{
			/*call swap function to percolate up*/
			swapDynArr(heap, parent, position);
			/*the position will now be in the parent position*/
			position = parent;
		}
		else return;
	}
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	takes parent node and then compares it to the smalles of its two children
	if root is larger than the child they are swaped*/
	...
	if (rightIdx < max)
	{
		...
		_minIdx = _smallerIndexHeap(heap, leftIdx, rightIdx);
		/*compare smallest child to the pos*/
		if (compare(getDynArr(heap,_minIdx), getDynArr(heap, pos))==-1)
		{
			/*call swap if necessary and recursivley call adjust heap*/
			swapDynArr(heap, pos, _minIdx);
			_adjustHeap(heap, max, _minIdx);
		}
	}
	else if (leftIdx < max)
	{
		/*compare min to position*/
...
void removeMinHeap(DynArr *heap)
{
	...
	/*taking away the root and replacing it with the very last element*/

	assert(sizeDynArr(heap) > 0);
	/*set last to the size of the array minus 1*/
	...
	/*remove the value at the end that just got placed at the root*/
	removeAtDynArr(heap, last);

	/*rebuild heat -> percolate down*/
...
void sortHeap(DynArr *heap)
{
	/*FIXME*/
	/*run build heap
	swap frist and last element
	call adjust heap from 0 up to last elemnt
