...
void loadList(DynArr *heap, FILE *filePtr)
{
	...
	/*while there is data in the file, create a new task*/
	while (fscanf(filePtr, "%d\t %[^\r\n]s", &priority, description) == 2)
	{	
...
void printList(DynArr *heap)
{
	...
	DynArr *tempHeap = newDynArr(heap->size);
	...
	{
		/*print the task*/
		printf("%d\t %s\n", tempHeap->data[i].priority, tempHeap->data[i].description);
		/*remove task from dynamic array*/
		removeAtDynArr(tempHeap, i);
	}
